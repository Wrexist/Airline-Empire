            ...
            AECard {
                VStack(alignment: .leading, spacing: AETheme.spacingS) {
                    ...
                    ForEach(offers, id: \.choice) { offer in
                        let deadline = Format.longDate(GameCalendar.date(at: offer.deadline, startYear: state.meta.startYear))
                        VStack(alignment: .leading, spacing: AETheme.spacingXS) {
                            Text(title(offer.kind)).font(.subheadline.weight(.semibold))
                            Text("Completion bonus: \(Format.money(offer.reward))")
                                .font(.caption)
                            Text("Deadline: \(deadline), 00:00")
                                .font(.caption).foregroundStyle(.secondary)
                            ConfirmableButton(title: "Accept this contract?",
                                              message: "\(title(offer.kind)) by \(deadline), 00:00. Earn \(Format.money(offer.reward)). Only activity after acceptance counts. This uses this month's offer.",
