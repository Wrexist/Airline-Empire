...
/// A brief route reveal above static geography. No timer or perpetual redraw;
/// only the lightweight route layer interpolates while the menu appears.
struct MenuRouteAtlas: View {
    ...
    var body: some View {
        Canvas { context, size in
            ...
            for (index, city) in MenuAtlasCities.all.enumerated() {
        ...
        .overlay {
            MenuRouteReveal(progress: reduceMotion || revealed ? 1 : 0)
        }
        .mask {
        ...
        .onAppear {
            withAnimation(reduceMotion ? nil : .easeInOut(duration: 1.8)) { revealed = true }
        }
    }
}

private enum MenuAtlasCities {
    static let all: [(String, Coordinate)] = [
...
private struct MenuRouteReveal: View, Animatable {
    var progress: Double
    ...
    var body: some View {
        Canvas { context, size in
            ...
            func point(_ coordinate: Coordinate) -> CGPoint {
                let value = MapPoint(coordinate: coordinate)
                return CGPoint(x: (value.x - 0.5) * scale + size.width / 2,
                               y: (value.y - 0.43) * scale / 2 + size.height / 2)
            }
            let origin = point(MenuAtlasCities.all[0].1)
            for (index, city) in MenuAtlasCities.all.dropFirst().enumerated() {
                let destination = point(city.1)
                var route = Path()
                ...
                let amount = min(1, max(0, progress * 1.3 - Double(index) * 0.15))
                let revealedRoute = route.trimmedPath(from: 0, to: amount)
                context.stroke(revealedRoute, with: .color(AETheme.ember.opacity(0.7)),
                               style: StrokeStyle(lineWidth: 1, dash: [3, 4]))
                if amount > 0, amount < 1, let tip = revealedRoute.currentPoint {
                    context.fill(Path(ellipseIn: CGRect(x: tip.x - 3, y: tip.y - 3,
                                                       width: 6, height: 6)),
                                 with: .color(AETheme.ember.opacity(0.9)))
                }
            }
        }
    }
