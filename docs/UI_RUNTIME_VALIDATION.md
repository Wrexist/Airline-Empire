...
| 136 | AE-043 (c127d2d) — docs and a headless tool only, app byte-identical to 135 | **failed twice, neither in the product.** Core: `followingTheAdviceFromNewYorkDoesNotBankruptThePlayer` tripped its 5-minute limit **after printing its result line** — every assertion had passed, with AE-042's own values. Measured alone the campaign takes **65.05 s**; Swift Testing measures a limit as wall clock while all 457 tests run in parallel, so five minutes was timing the runner. Raised to ten with the measurement in the test; no assertion touched. The arrival + shell shard starved on three AX/query timeouts, with `testDetailScreensAndSettingsRender` at **459.7 s** against 104.2 s in run 135. Campaign 6/6, economy 5/5, all builds clean. One redispatch |
| 137 | the same tree with the raised limit (38f5354) | **green — 457 Core tests, 20 journeys, both measurements, 0 failures on any shard.** Core 1,681.6 s with the release build clean; campaign 6/6 in 907.2 s; economy 5/5 in 792.6 s; arrival + shell **9/9**, the Munich arrival 425.7 s. The starvation canary settles run 136: `testDetailScreensAndSettingsRender` **78.4 s** here, 459.7 s in run 136, 104.2 s in run 135 — a six-fold spread on identical code |
