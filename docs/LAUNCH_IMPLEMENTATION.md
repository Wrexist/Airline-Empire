...
Candidate `df20a83` passed 523 Core release tests, the full Core debug/release-build gate, all five economy tests, all six campaign tests, the complete Munich arrival journey, shell/map (eight passes and its existing synthetic flight-selection skip), iPad, release tooling, all hosted save/StoreKit tests, free entry and recap. The current-simulator four-tab test also passed. First-flight observation still failed: the normal-speed window ended at 20:15 on the founding day, before `FlightSchedulingSystem`'s daily boundary had created flights. The journey now advances through the first midnight with the checked production day control, then observes real departures at 1x. The onboarding hint explains that schedule timing. The 26.2 native recording also confirms that a synthetic Pause tap at the reported centre left 1x selected. Speed selection now permits one state-checked idempotent retry, retaining a checkpoint and failing after two unsuccessful attempts. Purchases and calendar advances do not gain this retry. These final corrections still require validation.
