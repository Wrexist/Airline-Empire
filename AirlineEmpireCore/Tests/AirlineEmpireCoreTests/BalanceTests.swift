...
struct BalanceTests {
    ...
    func archetypeParityAndSanity() throws {
        ...
        // Every strategy must be viable, including the cheapest fares.
        // The old positive-only ratio silently omitted the dead low-cost
        // archetype. Measured ownership/spending fixes make all five
        // medians exceed their $120M starting capital across nine seeds.
        #expect(positives.count == AIArchetype.allCases.count,
                "A strategy has no positive median: \(medians)")
        for (archetype, median) in medians {
            #expect(median >= Money.dollars(120_000_000).cents,
                    "\(archetype) destroys starting capital: \(median)")
        }
