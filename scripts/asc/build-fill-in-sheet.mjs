...
w()
w('> Simulation and Strategy describe the game accurately. No additional category is needed; see [ASO rationale](ASO.md).')
w()
...
w('**IAP review screenshot:** prepared, inspected and uploaded-ready for all')
w('three products (1206 × 2622 RGB). `node scripts/asc/upload-iap-review.mjs --check`')
w('verifies them, and the metadata workflow fills any empty slot. App Review still')
w('needs each product **added to the version\'s submission** — Apple rejected 1.0 for')
w('that, not for a missing image. Verify products in StoreKit sandbox.')
w()
...
w('**Yes — Purchase History, for App Functionality and Analytics.**')
w()
w('RevenueCat processes anonymous purchase history. Mark it not linked to identity')
w('and not used for tracking. There is no advertising SDK or game account.')
w('Gameplay is local; Apple processes payments. Confirm the privacy')
...
w()
w('Portrait, PNG, **no alpha channel**, at most ten per size. Each image is the')
w('app itself, full screen, with a compact caption band — Review Guideline 2.3.3')
w('(rejection of 1.0) requires the majority of every screenshot to be the app, so')
w('the decorative illustration is not used here. The shot list and captions are in')
w('`docs/ASO.md` §5; the upload can be done for you by the metadata workflow with')
w('**screenshots** ticked.')
w()
...
w('- [ ] Age rating shows 4+ and App Privacy discloses anonymous Purchase History for App Functionality and Analytics, without tracking.')
w()
